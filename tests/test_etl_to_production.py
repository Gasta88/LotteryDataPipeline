...
class ETLtoProdTestCase(unittest.TestCase):
    ...
    def test_prepare_discount_table(self):
        ...
        with sqlite3.connect(db_staging_file) as conn:
            cur = conn.cursor()
            cur.executemany("INSERT INTO lottery_purchase_stg VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);",
                            lottery_content)
            conn.commit()
    ...
    def test_prepare_registration_table(self):
        ...
        registration_content = [('2018-01-01 00:03:00.3136468','websiteF','riio@uqkry-.org','1988-12-13 16:57:40.7219400','Justin Dorsey','Rickey Villa','960 Nobel Boulevard','40 Fabien Boulevard','Michigan','Arkansas','63 Second St.','2018-01-01 00:03:00.3136468','477952'),
    ...
    def test_prepare_ticket_table(self):
        ...
        with sqlite3.connect(db_staging_file) as conn:
            cur = conn.cursor()
            cur.executemany("INSERT INTO games_purchase_stg VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);",
                            games_content)
            # conn.commit()
            cur.executemany("INSERT INTO lottery_purchase_stg VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);",
                            lottery_content)
